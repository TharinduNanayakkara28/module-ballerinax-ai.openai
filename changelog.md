...
- [Add `chatStream`, which streams the model's answer as normalized `ai:ChatCompletionChunk` values - text, reasoning and tool-call fragments and a finish reason, carried directly on the chunk - over the Chat Completions or the Responses API, selected by `apiType`. Token usage is reported to the observability span rather than on the chunk](https://github.com/wso2/product-ballerina-integrator/issues/2457)
