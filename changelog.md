...
- [Add `chatAsStream`, which streams the model's answer as normalized `ai:ChatMessageChunk` values - `role` on every chunk, plus text, reasoning and tool-call fragments and a finish reason - over the Chat Completions or the Responses API, selected by `apiType`](https://github.com/wso2/product-ballerina-integrator/issues/2457)
- [Add `generateAsStream`, which streams the generated answer as `string` fragments; structured output has no valid intermediate state, so use `generate` for that](https://github.com/wso2/product-ballerina-integrator/issues/2457)
- [Streaming requests reach OpenAI under the caller's full `ConnectionConfig`, so a proxy or a custom truststore applies to `chatAsStream` exactly as it does to `chat`](https://github.com/wso2/product-ballerina-integrator/issues/2457)
